...
.title-row{display:flex;align-items:center;gap:8px;margin-bottom:5px;min-width:0}.title-view{display:flex;align-items:center;gap:7px;min-width:0}.title-row h1{font-size:1.22em;line-height:1.3;font-weight:600;margin:0;overflow-wrap:anywhere}.title-inline-editor{display:flex;align-items:center;gap:6px;flex:1;min-width:0}.title-prefix{font-size:1.22em;font-weight:600;white-space:nowrap}.title-inline-editor input{font-size:1.22em;font-weight:600;min-width:180px;max-width:720px}.state-dot{width:8px;height:8px;border-radius:50%;flex:0 0 8px;background:currentColor}.state-open{color:var(--success)}.state-closed{color:var(--danger)}.state-merged{color:var(--merged)}
...
@media(max-width:700px){body{padding:12px}.time{margin-left:0;width:100%}.commit-entry{grid-template-columns:max-content minmax(0,1fr)}.commit-author{grid-column:2}.title-inline-editor{flex-wrap:wrap}.title-inline-editor input{font-size:1em;max-width:100%}}
</style>
</head>
<body>
<header>
  <div class="title-row">
    ...
    <div id="title-view" class="title-view"><h1>Pull Request #${pr.number} ${escHtml(pr.title)}</h1><button id="edit-title" class="icon-btn" title="Edit title" aria-label="Edit pull request title">${editIcon}</button></div>
    <div id="title-editor" class="title-inline-editor" hidden><span class="title-prefix">Pull Request #${pr.number}</span><input id="title-input" type="text" aria-label="Pull request title" value="${escHtml(pr.title)}"><button id="save-title" class="btn small">Save</button><button id="cancel-title" class="btn small sec">Cancel</button></div>
  </div>
...
<section id="tab-discussion" class="tab-content active" role="tabpanel" aria-labelledby="discussion-tab">
  ...
  <div id="body-action-row" class="body-action-row"><button id="edit-body" class="btn small ghost">Edit description</button></div>
  ...
  <h2 class="section-heading">Comments (${comments.length})</h2><div>${commentsHtml}</div><div class="form-section"><textarea id="comment-body" aria-label="Add a comment" style="height:70px" placeholder="Write a comment..."></textarea><div class="editor-actions"><button id="post-comment" class="btn">Post Comment</button></div></div>
</section>
...
function setTitleEditing(editing){document.getElementById('title-view').hidden=editing;document.getElementById('title-editor').hidden=!editing;if(editing)document.getElementById('title-input')?.focus();}
function setBodyEditing(editing){document.getElementById('body-view').hidden=editing;document.getElementById('body-action-row').hidden=editing;document.getElementById('body-editor').hidden=!editing;if(editing)document.getElementById('body-input')?.focus();}
document.getElementById('edit-title')?.addEventListener('click',()=>setTitleEditing(true));document.getElementById('cancel-title')?.addEventListener('click',()=>setTitleEditing(false));document.getElementById('save-title')?.addEventListener('click',()=>{const title=(document.getElementById('title-input').value||'').trim();if(title)post('editTitle',{title});});document.getElementById('title-input')?.addEventListener('keydown',(event)=>{if(event.key==='Escape')setTitleEditing(false);if(event.key==='Enter'){event.preventDefault();document.getElementById('save-title')?.click();}});
