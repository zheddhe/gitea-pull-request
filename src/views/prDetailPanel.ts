...
export class PRDetailPanel {
  ...
  private async handleMessage(message: {
    ...
    switch (message.command) {
      ...
      case "navigateReviewNavigation": {
        const mode = message.mode === "pending" ? "pending" : "unresolved";
      ...
      case "requestReviewNavigationState": {
        const state = await vscode.commands.executeCommand(
          "gitea.getReviewNavigationState",
        );
      ...
      case "selectReviewNavigationItem": {
        const mode = message.mode === "pending" ? "pending" : "unresolved";
        const itemId =
          typeof message.itemId === "string" ? message.itemId : undefined;
        await vscode.commands.executeCommand(
  ...
  private renderReviewConversation(
    ...
    return `<div class="review-conversation${conversation.resolved ? " conversation-resolved conversation-collapsed" : ""}${conversation.outdated ? " conversation-outdated" : ""}${standalone ? " standalone" : ""}" data-root-comment-id="${root.id}" data-nav-id="conversation:${root.id}" data-persisted-resolved="${conversation.resolved ? "true" : "false"}">${resolvedEventHtml}${contentHtml}</div>`;
  }
  ...
  private renderHtml(
    ...
    const projectedReviewConversations = projectReviewConversationsForHead(
      reviewConversations,
      pr.head.sha,
    );
...
.review-navigation-toolbar{display:flex;align-items:center;justify-content:flex-start;gap:0;padding:6px 7px;background:var(--surface)}.review-navigation-toolbar[hidden]{display:none}.review-navigation-group{display:flex;align-items:center;gap:6px;padding:0 10px}.review-navigation-group:first-child{padding-left:3px}.review-navigation-group+ .review-navigation-group{border-left:1px solid var(--border);margin-left:4px;padding-left:14px}.review-navigation-group[hidden]{display:none}.review-navigation-label{color:var(--muted);font-size:.82em;font-weight:600}.review-navigation-actions{display:inline-flex;align-items:center;gap:1px}.review-navigation-actions[hidden]{display:none}.review-navigation-toolbar .icon-btn{width:26px;height:26px}.review-navigation-toolbar .icon-btn svg{width:16px;height:16px}.review-navigation-position{min-width:38px;text-align:center;color:var(--muted);font-size:.86em}.pending-navigation-group .review-navigation-label,.pending-navigation-group .review-navigation-position,.pending-navigation-group .icon-btn{color:var(--warning)}.review-conversation.review-navigation-target,[data-pending-id].review-navigation-target{outline:1px solid var(--focus);outline-offset:2px;box-shadow:0 0 0 2px color-mix(in srgb,var(--focus) 18%,transparent)}
...
<section id="tab-inline-reviews" class="tab-content active" role="tabpanel" aria-labelledby="inline-reviews-tab">
  <div id="review-fixed-footer" class="review-fixed-footer">
    ...
    <div id="review-navigation-toolbar" class="review-navigation-toolbar" hidden>
      <span id="unresolved-navigation-group" class="review-navigation-group unresolved-navigation-group" hidden><span class="review-navigation-label">Unresolved</span><span id="unresolved-navigation-position" class="review-navigation-position"></span><span id="unresolved-navigation-actions" class="review-navigation-actions"><button id="previous-unresolved" class="icon-btn" title="Previous unresolved conversation" aria-label="Previous unresolved review conversation">${REVIEW_PREVIOUS_ICON}</button><button id="next-unresolved" class="icon-btn" title="Next unresolved conversation" aria-label="Next unresolved review conversation">${REVIEW_NEXT_ICON}</button></span></span>
      <span id="pending-navigation-group" class="review-navigation-group pending-navigation-group" hidden><span class="review-navigation-label">Pending</span><span id="pending-navigation-position" class="review-navigation-position"></span><span id="pending-navigation-actions" class="review-navigation-actions"><button id="previous-pending" class="icon-btn" title="Previous pending review operation" aria-label="Previous pending review operation">${REVIEW_PREVIOUS_ICON}</button><button id="next-pending" class="icon-btn" title="Next pending review operation" aria-label="Next pending review operation">${REVIEW_NEXT_ICON}</button></span></span>
    </div>
  </div>
  ${replyCapabilityHtml}
  ${filesHtml}
...
let reviewNavigationState=savedState.reviewNavigationState&&typeof savedState.reviewNavigationState==='object'?savedState.reviewNavigationState:{mode:'unresolved',activeItemId:null};
...
function persistReviewNavigation(){vscode.setState(Object.assign({},vscode.getState()||{},{reviewNavigationState:reviewNavigationState}));}
...
function pendingReviewElements(){return Array.from(document.querySelectorAll('[data-pending-id][data-nav-id]'));}
function navigationElements(mode){return mode==='pending'?pendingReviewElements():unresolvedReviewConversations();}
function navigationElementById(itemId){if(!itemId)return null;return Array.from(document.querySelectorAll('[data-nav-id]')).find((item)=>item.dataset.navId===itemId)||null;}
function navigationPosition(mode){const items=navigationElements(mode);const activeId=reviewNavigationState.mode===mode?reviewNavigationState.activeItemId:null;const index=activeId?items.findIndex((item)=>item.dataset.navId===activeId):-1;return {items,index};}
function updateReviewNavigation(){const unresolved=navigationPosition('unresolved');const pending=navigationPosition('pending');const toolbar=document.getElementById('review-navigation-toolbar');const unresolvedGroup=document.getElementById('unresolved-navigation-group');const pendingGroup=document.getElementById('pending-navigation-group');const unresolvedActions=document.getElementById('unresolved-navigation-actions');const pendingActions=document.getElementById('pending-navigation-actions');const unresolvedPosition=document.getElementById('unresolved-navigation-position');const pendingPosition=document.getElementById('pending-navigation-position');if(unresolvedGroup)unresolvedGroup.hidden=unresolved.items.length===0;if(pendingGroup)pendingGroup.hidden=pending.items.length===0;if(unresolvedActions)unresolvedActions.hidden=unresolved.items.length<=1;if(pendingActions)pendingActions.hidden=pending.items.length<=1;if(toolbar)toolbar.hidden=unresolved.items.length===0&&pending.items.length===0;if(unresolvedPosition)unresolvedPosition.textContent=unresolved.items.length<=1?String(unresolved.items.length):(unresolved.index>=0?(unresolved.index+1)+' / '+unresolved.items.length:String(unresolved.items.length));if(pendingPosition)pendingPosition.textContent=pending.items.length<=1?String(pending.items.length):(pending.index>=0?(pending.index+1)+' / '+pending.items.length:String(pending.items.length));syncReviewFooter();}
...
function revealNavigationElement(element){if(!element)return;const conversation=element.classList.contains('review-conversation')?element:element.closest('.review-conversation');if(conversation){revealReviewConversation(conversation);if(element!==conversation){element.classList.add('review-navigation-target');if(reviewNavigationHighlightTimer)clearTimeout(reviewNavigationHighlightTimer);reviewNavigationHighlightTimer=setTimeout(()=>element.classList.remove('review-navigation-target'),1400);}return;}const diff=element.closest('.file-diff');if(diff&&diff.hidden){diff.hidden=false;const block=diff.closest('.file-block');const header=block?.querySelector('[data-file-toggle]');if(header)header.setAttribute('aria-expanded','true');}document.querySelectorAll('.review-navigation-target').forEach((item)=>item.classList.remove('review-navigation-target'));element.classList.add('review-navigation-target');if(reviewNavigationHighlightTimer)clearTimeout(reviewNavigationHighlightTimer);reviewNavigationHighlightTimer=setTimeout(()=>element.classList.remove('review-navigation-target'),1400);element.scrollIntoView({block:'center',behavior:'smooth'});}
function applyReviewNavigationState(state){const source=state&&typeof state==='object'?state:{};reviewNavigationState={mode:source.mode==='pending'?'pending':'unresolved',activeItemId:typeof source.activeItemId==='string'?source.activeItemId:null};persistReviewNavigation();updateReviewNavigation();const active=navigationElementById(reviewNavigationState.activeItemId);if(active)revealNavigationElement(active);}
function navigateReview(mode,direction){post('navigateReviewNavigation',{mode:mode,direction:direction});}
function selectNavigationElement(element,mode){const itemId=element?.dataset?.navId;if(!itemId)return;post('selectReviewNavigationItem',{mode:mode,itemId:itemId});}
...
document.querySelectorAll('.review-conversation[data-nav-id]').forEach((conversation)=>conversation.addEventListener('click',()=>selectNavigationElement(conversation,'unresolved')));
document.getElementById('previous-unresolved')?.addEventListener('click',()=>navigateReview('unresolved',-1));document.getElementById('next-unresolved')?.addEventListener('click',()=>navigateReview('unresolved',1));document.getElementById('previous-pending')?.addEventListener('click',()=>navigateReview('pending',-1));document.getElementById('next-pending')?.addEventListener('click',()=>navigateReview('pending',1));
