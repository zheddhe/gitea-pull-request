...
.review-navigation-toolbar{display:flex;align-items:center;justify-content:flex-start;gap:10px;padding:6px 7px;background:var(--surface)}.review-navigation-toolbar[hidden]{display:none}.review-navigation-group{display:flex;align-items:center;gap:3px}.review-navigation-group[hidden]{display:none}.review-navigation-label{color:var(--muted);font-size:.82em;font-weight:600;margin-right:2px}.review-navigation-toolbar .icon-btn{width:26px;height:26px}.review-navigation-toolbar .icon-btn svg{width:16px;height:16px}.review-navigation-position{min-width:76px;text-align:center;color:var(--muted);font-size:.86em}.review-conversation.review-navigation-target,[data-pending-id].review-navigation-target{outline:1px solid var(--focus);outline-offset:2px;box-shadow:0 0 0 2px color-mix(in srgb,var(--focus) 18%,transparent)}
...
<section id="tab-inline-reviews" class="tab-content active" role="tabpanel" aria-labelledby="inline-reviews-tab">
  <div id="review-fixed-footer" class="review-fixed-footer">
    ...
    <div id="review-navigation-toolbar" class="review-navigation-toolbar" hidden>
      <span id="unresolved-navigation-group" class="review-navigation-group" hidden><span class="review-navigation-label">Unresolved</span><button id="previous-unresolved" class="icon-btn" title="Previous unresolved conversation" aria-label="Previous unresolved review conversation">${REVIEW_PREVIOUS_ICON}</button><span id="unresolved-navigation-position" class="review-navigation-position"></span><button id="next-unresolved" class="icon-btn" title="Next unresolved conversation" aria-label="Next unresolved review conversation">${REVIEW_NEXT_ICON}</button></span>
      <span id="pending-navigation-group" class="review-navigation-group" hidden><span class="review-navigation-label">Pending</span><button id="previous-pending" class="icon-btn" title="Previous pending review operation" aria-label="Previous pending review operation">${REVIEW_PREVIOUS_ICON}</button><span id="pending-navigation-position" class="review-navigation-position"></span><button id="next-pending" class="icon-btn" title="Next pending review operation" aria-label="Next pending review operation">${REVIEW_NEXT_ICON}</button></span>
    </div>
  </div>
  ${replyCapabilityHtml}
  ${filesHtml}
...
function updateReviewNavigation(){const unresolved=navigationPosition('unresolved');const pending=navigationPosition('pending');const toolbar=document.getElementById('review-navigation-toolbar');const unresolvedGroup=document.getElementById('unresolved-navigation-group');const pendingGroup=document.getElementById('pending-navigation-group');const unresolvedPosition=document.getElementById('unresolved-navigation-position');const pendingPosition=document.getElementById('pending-navigation-position');if(unresolvedGroup)unresolvedGroup.hidden=unresolved.items.length===0;if(pendingGroup)pendingGroup.hidden=pending.items.length===0;if(toolbar)toolbar.hidden=unresolved.items.length===0&&pending.items.length===0;if(unresolvedPosition)unresolvedPosition.textContent=unresolved.index>=0?(unresolved.index+1)+' / '+unresolved.items.length:String(unresolved.items.length);if(pendingPosition)pendingPosition.textContent=pending.index>=0?(pending.index+1)+' / '+pending.items.length:String(pending.items.length);syncReviewFooter();}
