...
.title-row{display:flex;align-items:center;gap:7px;margin-bottom:5px;min-width:0}.title-row h1{display:flex;align-items:center;gap:5px;min-width:0;font-size:1.22em;line-height:1.3;font-weight:600;margin:0}.title-text{overflow-wrap:anywhere}.title-input{display:none;width:min(720px,60vw);font-size:1em;font-weight:600;padding:1px 5px}.title-row.editing .title-text{display:none}.title-row.editing .title-input{display:inline-block}.state-dot{width:8px;height:8px;border-radius:50%;flex:0 0 8px;background:currentColor}.state-open{color:var(--success)}.state-closed{color:var(--danger)}.state-merged{color:var(--merged)}
...
.branch-tag{font-family:var(--mono);font-size:.92em;background:var(--surface-subtle);border:1px solid var(--border);border-radius:2px;padding:1px 6px;color:var(--fg)}.branch-select{font-family:var(--mono);font-size:.92em;min-height:24px;padding:1px 24px 1px 6px;background:var(--surface-subtle);color:var(--fg);border:1px solid var(--border);border-radius:2px}.review-state{display:inline-flex;align-items:center;border:1px solid currentColor;border-radius:999px;padding:1px 7px;font-size:.82em;font-weight:600}.review-approved{color:var(--success)}.review-changes-requested{color:var(--danger)}.review-pending{color:var(--warning)}.context-separator{color:var(--border)}.context-stat{display:inline-flex;gap:3px}.context-stat strong{font-weight:600}.additions{color:var(--success)}.deletions{color:var(--danger)}
.actions{display:flex;gap:6px;flex-wrap:wrap;margin-bottom:14px}.btn{border:1px solid transparent;border-radius:2px;padding:4px 10px;min-height:26px;cursor:pointer;background:var(--button-bg);color:var(--button-fg)}.btn:hover{background:var(--button-hover)}.btn.sec{background:var(--button-secondary-bg);color:var(--button-secondary-fg)}.btn.sec:hover{background:var(--button-secondary-hover)}.btn.danger{background:var(--button-secondary-bg);color:var(--danger);border-color:color-mix(in srgb,var(--danger) 55%,transparent)}.btn.success{background:var(--button-secondary-bg);color:var(--success);border-color:color-mix(in srgb,var(--success) 55%,transparent)}.btn.small{padding:2px 8px;min-height:22px;font-size:.9em}.icon-btn{display:inline-flex;align-items:center;justify-content:center;width:22px;height:22px;padding:2px;border:0;border-radius:3px;background:transparent;color:var(--muted);cursor:pointer}.icon-btn:hover{background:var(--vscode-toolbar-hoverBackground,var(--surface-subtle));color:var(--fg)}.icon-btn svg{width:14px;height:14px}.btn:focus-visible,.icon-btn:focus-visible,.tab:focus-visible,input:focus-visible,textarea:focus-visible,select:focus-visible,.file-header:focus-visible,a:focus-visible{outline:1px solid var(--focus);outline-offset:2px}
...
<header>
  <div id="title-row" class="title-row">
    ...
    <h1><span>Pull Request #${pr.number}</span><span id="title-text" class="title-text">${escHtml(pr.title)}</span><input id="title-input" class="title-input" type="text" aria-label="Pull request title" value="${escHtml(pr.title)}"></h1>
  ...
  <div class="context-row">
    ...
    <span class="branch-tag">${escHtml(pr.head.ref)}</span><span>→</span>${isOpen ? `<select id="base-select" class="branch-select" aria-label="Target branch">${branchOptions}</select>` : `<span class="branch-tag">${escHtml(pr.base.ref)}</span>`}
...
<section id="tab-discussion" class="tab-content active" role="tabpanel" aria-labelledby="discussion-tab">
  <section class="section-card">
    <div class="section-bar"><span>Description</span><button id="edit-body" class="icon-btn" title="Edit description" aria-label="Edit pull request description">${editIcon}</button></div>
    <div id="body-view" class="section-content">${bodyHtml ? `<div class="markdown-body">${bodyHtml}</div>` : '<div class="empty">(no description)</div>'}</div>
    <div id="body-editor" class="description-editor" hidden><textarea id="body-input">${escHtml(pr.body || "")}</textarea><div class="editor-actions"><button id="save-body" class="btn">Save</button><button id="cancel-body" class="btn sec">Cancel</button></div></div>
  </section>
  <section>
    <div class="section-bar"><span>Comments (${comments.length})</span></div>
    <div style="padding-top:10px">${commentsHtml}</div>
    <div class="form-section"><textarea id="comment-body" aria-label="Add a comment" style="height:70px" placeholder="Write a comment..."></textarea><div class="editor-actions"><button id="post-comment" class="btn">Post Comment</button></div></div>
  </section>
</section>
<section id="tab-reviews" class="tab-content" role="tabpanel" aria-labelledby="reviews-tab"><div class="review-submit"><strong>Submit Review <span id="pending-count" class="muted"></span></strong><textarea id="review-body" style="height:70px;margin-top:8px" placeholder="Overall review comment (optional)..."></textarea><div class="review-actions">${isOpen ? '<button class="btn success" data-review-event="APPROVED">Approve</button><button class="btn danger" data-review-event="REQUEST_CHANGES">Request Changes</button>' : ""}<button class="btn" data-review-event="COMMENT">Comment</button></div><div class="review-batch-actions"><button id="submit-inline" class="btn sec submit-inline">Submit inline comments</button><span id="review-batch-hint" class="review-batch-hint">Inline comments are queued until you submit a review.</span></div></div><div class="files-summary">${files.length} file(s) changed · Select a diff line to add an inline review comment</div>${filesHtml}<div class="section-bar" style="margin-top:14px"><span>Submitted Reviews</span></div><div style="padding-top:10px">${reviewsHtml}</div></section>
<section id="tab-commits" class="tab-content" role="tabpanel" aria-labelledby="commits-tab">${commitsHtml}</section>
<script nonce="${nonce}">
const vscode=acquireVsCodeApi();let pendingComments=[];let openFormKey=null;let originalTitle=${JSON.stringify(pr.title)};let cancelTitle=false;
...
function beginTitleEdit(){const row=document.getElementById('title-row');const input=document.getElementById('title-input');cancelTitle=false;row?.classList.add('editing');input?.focus();input?.select();}
function finishTitleEdit(save){const row=document.getElementById('title-row');const input=document.getElementById('title-input');const text=document.getElementById('title-text');if(!input)return;const next=(input.value||'').trim();row?.classList.remove('editing');if(!save||!next){input.value=originalTitle;if(text)text.textContent=originalTitle;return;}if(next===originalTitle)return;if(text)text.textContent=next;post('editTitle',{title:next});}
function setBodyEditing(editing){document.getElementById('body-view').hidden=editing;document.getElementById('body-editor').hidden=!editing;document.getElementById('edit-body').hidden=editing;if(editing)document.getElementById('body-input')?.focus();}
document.getElementById('edit-title')?.addEventListener('click',beginTitleEdit);document.getElementById('title-input')?.addEventListener('keydown',(event)=>{if(event.key==='Escape'){cancelTitle=true;finishTitleEdit(false);event.preventDefault();}else if(event.key==='Enter'){finishTitleEdit(true);event.preventDefault();}});document.getElementById('title-input')?.addEventListener('blur',()=>{if(cancelTitle){cancelTitle=false;return;}finishTitleEdit(true);});
document.getElementById('edit-body')?.addEventListener('click',()=>setBodyEditing(true));document.getElementById('cancel-body')?.addEventListener('click',()=>setBodyEditing(false));document.getElementById('save-body')?.addEventListener('click',()=>post('editBody',{body:document.getElementById('body-input').value||''}));
document.getElementById('base-select')?.addEventListener('change',(event)=>{const base=event.target.value;if(base)post('updateBase',{base});});
